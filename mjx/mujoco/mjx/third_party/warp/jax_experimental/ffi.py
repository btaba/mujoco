...
class GraphMode(IntEnum):
    ...
    WARP_FORCE_CAPTURE = 4  # let Warp capture a graph on every call
    ...
    def ffi_callback(self, call_frame):
        try:
            ...
            with _FFI_CALLBACK_LOCK:
                ...
                if self.graph_mode == GraphMode.WARP or self.graph_mode == GraphMode.WARP_FORCE_CAPTURE:
                    ...
                    if self.graph_mode == GraphMode.WARP_FORCE_CAPTURE:
                        capture = None
                    else:
                        capture = self.captures.get(capture_key)
                ...
                with wp.ScopedStream(stream, sync_enter=True):
                    ...
                    elif self.graph_mode == GraphMode.WARP or self.graph_mode == GraphMode.WARP_FORCE_CAPTURE:
                        # capturing with WARP
