...
class Context(PyTreeNode):
  ...
  def create(cls, m: Model, d: Data, grad: bool = True) -> 'Context':
    jaref = d._impl.efc_J @ d.qacc - d._impl.efc_aref  # pytype: disable=attribute-error
    # TODO(robotics-team): determine nv at which sparse mul is faster
    ma = support.mul_m(m, d, d.qacc)
    nv_0 = jp.zeros(m.nv)
    fri = 0.0
    if m.opt.cone == ConeType.ELLIPTIC:
      friction = d._impl.contact.friction[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
      dim = d._impl.contact.dim[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
    ...
    ctx = Context(
        ...
        Jaref=jaref,
        efc_force=d._impl.efc_force,  # pytype: disable=attribute-error
        Ma=ma,
        grad=nv_0,
        Mgrad=nv_0,
        search=nv_0,
...
class _LSPoint(PyTreeNode):
  ...
  def create(
    ...
    active = (x < 0).at[: d._impl.ne + d._impl.nf].set(True)  # pytype: disable=attribute-error

    dof_fl, ten_fl = m._impl.dof_hasfrictionloss, m._impl.tendon_hasfrictionloss  # pytype: disable=attribute-error
    if (dof_fl.any() or ten_fl.any()) and not (
        m.opt.disableflags & DisableBit.FRICTIONLOSS
    ):
      f = d._impl.efc_frictionloss  # pytype: disable=attribute-error
      r = 1.0 / (d._impl.efc_D + (d._impl.efc_D == 0.0) * mujoco.mjMINVAL)  # pytype: disable=attribute-error
    ...
    if m.opt.cone == ConeType.ELLIPTIC:
      ...
      dim1 = d._impl.contact.efc_address[d._impl.contact.dim == 1]  # pytype: disable=attribute-error
      nefl = d._impl.ne + d._impl.nf + d._impl.nl  # pytype: disable=attribute-error
      ...
      efc_elliptic = d._impl.contact.efc_address[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
...
def _update_constraint(m: Model, d: Data, ctx: Context) -> Context:
  ...
  active = (ctx.Jaref < 0).at[: d._impl.ne + d._impl.nf].set(True)  # pytype: disable=attribute-error

  floss_force, floss_cost = jp.zeros(d._impl.nefc), 0.0  # pytype: disable=attribute-error
  dof_fl, ten_fl = m._impl.dof_hasfrictionloss, m._impl.tendon_hasfrictionloss  # pytype: disable=attribute-error
  if (dof_fl.any() or ten_fl.any()) and not (
      m.opt.disableflags & DisableBit.FRICTIONLOSS
  ):
  ...
  if m.opt.cone == ConeType.PYRAMIDAL:
    efc_force = d._impl.efc_D * -ctx.Jaref * active + floss_force  # pytype: disable=attribute-error
    cost = 0.5 * jp.sum(d._impl.efc_D * ctx.Jaref * ctx.Jaref * active)  # pytype: disable=attribute-error
    dm, u, h = 0.0, 0.0, 0.0
  elif m.opt.cone == ConeType.ELLIPTIC:
    friction = d._impl.contact.friction[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
    efc_address = d._impl.contact.efc_address[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
    dim = d._impl.contact.dim[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
    ...
    dm = d._impl.efc_D[efc_address] / jp.maximum(  # pytype: disable=attribute-error
  ...
  qfrc_constraint = d._impl.efc_J.T @ efc_force  # pytype: disable=attribute-error
...
def _update_gradient(m: Model, d: Data, ctx: Context) -> Context:
  ...
  if m.opt.solver == SolverType.CG:
    ...
    if m.opt.cone == ConeType.ELLIPTIC:
      cm = jp.diag(d._impl.efc_D * ctx.active)  # pytype: disable=attribute-error
      efc_address = d._impl.contact.efc_address[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
      dim = d._impl.contact.dim[d._impl.contact.dim > 1]  # pytype: disable=attribute-error
      ...
      h = d._impl.efc_J.T @ cm @ d._impl.efc_J  # pytype: disable=attribute-error
    else:
      h = (d._impl.efc_J.T * d._impl.efc_D * ctx.active) @ d._impl.efc_J  # pytype: disable=attribute-error
...
def _linesearch(m: Model, d: Data, ctx: Context) -> Context:
  ...
  jv = d._impl.efc_J @ ctx.search  # pytype: disable=attribute-error
  ...
  quad = (quad * d._impl.efc_D).T  # pytype: disable=attribute-error
  uu, v0, uv, vv = 0.0, 0.0, 0.0, 0.0
  if m.opt.cone == ConeType.ELLIPTIC:
    mask = d._impl.contact.dim > 1  # pytype: disable=attribute-error
    # complete vector quadratic (for bottom zone)
    efc_con, efc_fri = [], []
    for condim, addr in zip(d._impl.contact.dim[mask], d._impl.contact.efc_address[mask]):  # pytype: disable=attribute-error
    ...
    efc_elliptic = d._impl.contact.efc_address[mask]  # pytype: disable=attribute-error
