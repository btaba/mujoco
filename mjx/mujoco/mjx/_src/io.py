...
from google3.learning.deepmind.analytics import usage_logging  # copybara:strip


def _assert_jax_device(device: jax.Device):
  if device is None:
    raise ValueError('No device specified.')

  if not isinstance(device, jax.Device):
    raise ValueError(
        f'Device "{device}" of type {type(device)} is not of type jax.Device.'
    )


def _is_cuda_gpu_device(device: jax.Device) -> bool:
  _assert_jax_device(device)
  try:
...
def _resolve_backend_impl(
  ...
  if device.platform == 'cpu':
    ...
    if mjx_c_default:
      logging.info('Picking default backend implementation: C.')
...
def _resolve_device(
  ...
  if backend_impl == types.BackendImpl.JAX:
    ...
    logging.info('Picking default device: %s.', device_0)
    return device_0

  if backend_impl == types.BackendImpl.C:
    cpu_0 = jax.devices('cpu')[0]
    logging.info('Picking default device: %s', cpu_0)
    return cpu_0
  ...
  if backend_impl == types.BackendImpl.WARP:
    ...
    logging.info('Picking default device: %s', cuda_gpus[0])
    return cuda_gpus[0]
...
def _check_backend_impl_device_compatibility(
  ...
  if backend_impl == types.BackendImpl.WARP:
    if not _is_cuda_gpu_device(device):
      raise AssertionError(
          'Warp backend implementation requires an CUDA GPU device, got '
          f'{device}.'
      )
...
def _resolve_backend_impl_and_device(
  ...
  match (has_backend_impl, has_device):
    case (True, True):
      pass
    case (True, False):
...
def _put_model_c(
  ...
  c_impl_obj = types.ModelC(**c_impl_dict)
...
def put_model(
  ...
  usage_logging.log_event(usage_logging.Event.MUJOCO, 'mjx.put_model')
  # copybara:strip_end
