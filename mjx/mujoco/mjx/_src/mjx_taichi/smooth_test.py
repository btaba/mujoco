...
class SmoothTest(absltest.TestCase):
  ...
  def test_kinematics(self):
    ...
    dx.qpos.from_numpy(qpos)

    smooth.kinematics(mx, dx)
    _assert_eq(d.xanchor, dx.xanchor.to_numpy().reshape((-1, 3)), 'xanchor')
    _assert_eq(d.xaxis, dx.xaxis.to_numpy().reshape((-1, 3)), 'xaxis')
    _assert_eq(d.xquat, dx.xquat.to_numpy().reshape((-1, 4)), 'xquat')
    _assert_eq(d.xpos, dx.xpos.to_numpy().reshape((-1, 3)), 'xpos')
