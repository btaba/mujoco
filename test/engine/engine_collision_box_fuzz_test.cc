...
// all contacts in a manifold share one normal, the manifold has at most four
// points, and no contact is deeper than the true penetration depth.
...
// maximum contacts the box-box collider may emit
...
mjtNum BruteForceSep(const mjModel* model, const mjData* data) {
  ...
  for (int gi = 0; gi < kSweepDirections; gi++) {
    ...
    mjtNum st = std::sqrt(std::max(0.0, 1 - ct * ct));
  ...
  for (int it = 0; it < n_configs; it++) {
    ...
    if (nbox > 0) {
      ...
      for (int i = 1; i < nbox; i++) {
        if (mju_dot3(box_con[0].normal, box_con[i].normal) < 1 - 1e-9) {
          st.mixed_normal++;
          break;
        }
      }
